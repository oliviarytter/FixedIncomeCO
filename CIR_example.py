...
r0, a, b, sigma = 0.03, 0.6, 0.05, 0.08
T_max, T_mesh = 10, 0.1
...
# 3m forward Libor rates
T_3m = np.array([i*0.25 for i in range(int(T_max/0.25)+1)])
p_3m = fid.zcb_price_cir(r0,a,b,sigma,T_3m)
f_3m = fid.forward_rates_from_zcb_prices(T_3m,p_3m,horizon = 1,type = "simple")

# Swap rates for a swap paying 3M Libor against an annual fixed rate
T_swap = np.array([i for i in range(0,T_max+1)])
R_swap = np.nan*np.ones([T_max+1])
for i in range(1,T_max+1):
    R_swap[i], S = fid.swap_rate_from_zcb_prices(0,0,T_swap[i],"annual",T,p,"quarterly",L = 0)
...
lb_sd = fid.ci_cir(r0,a,b,sigma,np.inf,size_ci,type_ci = "lower")[0]
ub_sd = fid.ci_cir(r0,a,b,sigma,np.inf,size_ci,type_ci = "upper")[1]
...
p4 = ax2.scatter(T_3m, f_3m, s = 2, color = 'orange', marker = ".",label="3M forward rate")
plots = [p1,p2,p3,p4]
labels = [item.get_label() for item in plots]
ax.legend(plots,labels,loc="lower right",fontsize = 5)
# fig.savefig("C:/Jacob/Uni_of_CPH/Interest rate derivatives/exam_prep/ZCB_prices_rates_plot.pdf")

# PLot of zcb prices, spot rates and forward rates in the Vasicek model
fig = plt.figure(constrained_layout=False,dpi=300,figsize=(5,3))
fig.suptitle(f"Swap- and 6M forward Libor rates in the CIR model",fontsize=10)
gs = fig.add_gridspec(nrows=1,ncols=1,left=0.12,bottom=0.2,right=0.88,top=0.90,wspace=0,hspace=0)
ax = fig.add_subplot(gs[0,0])
xticks = [0,2,4,6,8,10]
ax.set_xticks(xticks)
...
ax.set_ylim([yticks[0],yticks[-1] + (yticks[-1]-yticks[0])*0.02])
plt.grid(axis = 'y', which='major', color=(0.7,0.7,0.7,0), linestyle='--')
ax.set_ylabel(f"Swap rates and 3M forward CIR rates",fontsize = 7)
p1 = ax.scatter(T_swap, R_swap, s = 2, color = 'black', marker = ".",label="Swap rates")
p2 = ax.scatter(T_3m, f_3m, s = 2, color = 'orange', marker = ".",label="3M forward rate")
plots = [p1,p2]
labels = [item.get_label() for item in plots]
ax.legend(plots,labels,loc="lower right",fontsize = 5)
